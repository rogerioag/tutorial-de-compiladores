...
def p_operador_relacional(p):
    ...
    if p[1] == "<":
        ...
        filho = MyNode(name='diferenca', type='DIFERENTE', parent=pai)
        filho_sym = MyNode(name=p[1], type='SIMBOLO', parent=filho)
    elif p[1] == "<=":
...
def p_operador_soma(p):
    ...
    if p[1] == "+":
        ...
        adicao_lexema = MyNode(name='+', type='SIMBOLO', parent=mais)
        p[0] = MyNode(name='operador_soma', type='OPERADOR_SOMA', children=[mais])
    else:
       menos = MyNode(name='menos', type='MENOS')
       subtracao_lexema = MyNode(name='-', type='SIMBOLO', parent=menos)
...
def p_operador_multiplicacao(p):
    ...
    if p[1] == "*":
       ...
       divisao_lexema = MyNode(name=p[1], type='SIMBOLO', parent=divide)
...
def p_fator_error(p):
    """fator : error ABRE_PARENTESE expressao FECHA_PARENTESE
